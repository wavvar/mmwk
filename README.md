...
**Host CLI:** The CLI supports macOS/Linux POSIX shells through `./cli/run.sh` and Windows PowerShell through `./cli/run.ps1`. The POSIX wrapper manages its own `./cli/venv`; the PowerShell wrapper uses the active/system Python 3.10+ environment, so install `./cli/requirements.txt` first. Task helpers also have PowerShell wrappers with documented parity limits in the [CLI README](./cli/docs/en/README.md#host-platform-entry-points).
