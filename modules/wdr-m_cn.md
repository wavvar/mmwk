...
| 功能块 | 说明 |
| --- | --- |
| `ES8311` 音频编解码器 | 负责数字音频与模拟音频之间的转换，图中标注的 `I2C ADDRESS: 0011 000` 对应其控制地址配置 |
| 模拟输出 | `OUTP` / `OUTN` 经耦合与滤波网络后形成 `DAC_AOUTLP` / `DAC_AOUTLN`，送往后级功放 |
| `NS4150` 功放 | 将编解码器输出进一步放大，并通过 `PA_OUTL+` / `PA_OUTL-` 驱动扬声器接口 |
| 扬声器接口 | 原理图中的 `P1` 为扬声器连接点，适合外接喇叭执行提示音或语音播报 |
| 功放控制 | `ESP32_AU_PA_CTL_IO17` 接到功放控制脚，可用于由主控实现放音使能控制 |

图中下方还标出了 `AEC` 参考网络，说明设计中已考虑将播放参考信号引入音频处理链路，便于后续做回声消除或语音交互相关算法集成。
...
| 功能块 | 说明 |
| --- | --- |
| `ES7210` 麦克风采集前端 | 负责多路模拟麦克风输入的放大与模数转换，图中标注的 `I2C ADDRESS: 1000 000` 对应其地址配置 |
| `MICBIAS` 偏置 | 原理图中给出了 `MICBIAS12` 和 `MICBIAS34`，用于给驻极体麦克风等前端提供偏置电压 |
| 麦克风接口 | 当前图中明确画出了 `MIC1` 和 `MIC2` 两个接口座，分别接入差分音频输入 |
| 扩展输入通道 | `MIC3` / `MIC4` 以差分信号网络形式保留在原理图中，说明方案层面支持继续扩展更多采集通道 |
| 主控数据回读 | 采集后的数字音频经 `SDOUT1/TDMOUT` 输出到 `ESP32-S3` 的 `I2S` 接收侧，用于本地识别、回传或算法处理 |
