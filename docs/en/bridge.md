...
Choose this path if `node info` already reports bridge identity and you want the first validated end-to-end bridge workflow.
...
- `modes` is the capability list exposed by the active profile on radar-facing status surfaces.
- `fw.boot_mode` is the runtime radar boot path (`flash`, `host`, `uart`, `spi`).
- Bridge reports `modes: ["auto", "host"]`.
...
- Firmware: bridge firmware, **BRIDGE** mode. `node info.name` typically reports `mmwk_sensor_bridge`.
...
`radar status` reports the saved/configured `mode` together with `modes`, while `node info.fw.boot_mode` reports the live radar boot path for the current session.
...
- `node info` returns complete identity payload with all expected fields populated.
...
| `raw_auto` not working | Confirm `mqtt=1` and MQTT transport is connected before checking. |
