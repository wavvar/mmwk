...
- `node agent --mqtt 1 --raw-auto 1`
  - 当 bridge 的 agent key 缺失时，默认值已经是 `mqtt=1`、`raw_auto=1`，因此执行 `network mqtt` 并重启就足以建立 MQTT 控制。
  - 但在这次验证里，我们只观察到 fresh baseline 已经报告了 `mqtt=1` 和 `raw_auto=1`，并没有单独证明 NVS 中这些 key 是否缺失。
...
- fresh device 的初始状态很关键。
  - 按当前 main，fresh factory image 启动后应表现为 `mqtt=1`、`raw_auto=1`、`uri` 为空、`state=prov_waiting`、`ready=false`、`ip` 为空。
...
- 雷达重启动作需要显式 ready gate。
  - 对 `radar fw flash`、`radar fw ota`、`radar config apply`，以及 factory / baseline 恢复后的第一次上电，都要轮询 `radar status`，直到返回 `running`。
  - 不要用固定 sleep 去替代这个 gate。
- 本次环境里，`radar fw version` 不是可靠的成功判断。
  - 它返回的内容与 `node info` 相同，而不是独立的雷达固件版本字符串。
...
这是一个很关键的 fresh-device 基线：设备仍在等待 Wi-Fi 配网，但 fresh bridge 默认已经保持 `mqtt=1` 和 `raw_auto=1`。
...
- 在 2026-04-01 的复跑里，`radar fw ota` 自身在 phase 3 grace 后就返回了成功，随后立即补查的 `radar status` 已经是 `{"state":"running"}`。

结论：

- 即使 `radar fw ota` 已经返回成功，也要继续轮询 `radar status`，直到看到 `running`。
- 如果 `radar fw ota` 超时，不要立刻重刷。
- 对 `radar fw flash`、`radar config apply`，以及 factory / baseline 恢复路径里的第一次上电，也要使用同样的 `radar status = running` gate。
- 后续再用 `raw_resp` 中的启动输出确认雷达已真正启动。
- 在 `radar fw ota` 步骤直接用 `--raw-resp-output` 抓 `raw_resp` 只能算 best-effort。2026-03-19 的 main 分支验证里，这段窗口抓到了 `2` 条消息 / `128` 字节；而 2026-04-01 的成功复跑里，这段窗口则是空的。
...
- fresh bridge 现在应在执行 `network mqtt` 并重启后直接具备 MQTT 控制；`node agent --mqtt 1 --raw-auto 1` 仅保留为旧持久化值场景下的手动 override / 排障路径。
- OTA 前，MQTT 侧的 `radar status` 可能已经是 `updating`，也可能已经是 `running`；这两种情况都在验证里出现过，而且都不会阻塞后续 OTA。
- 设备成功通过 Wi-Fi 使用本地 HTTP 获取 `.bin` 和 `.cfg`。
- 多次验证里，`radar fw ota` 的完成路径不同：2026-03-19 这次先 timeout、后续轮询恢复到 `running`；2026-04-01 复跑则在内建 phase 3 grace 后于 `148.4` 秒直接成功返回。两种情况都不需要重刷。
