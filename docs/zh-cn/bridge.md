...
- `start_mode` 表示由雷达状态面暴露的、当前保存/配置的默认模式。
...
返回字段包括：`name`、`board`、`version`、`id`、`ip`、`mqtt_uri`、`client_id`、`cmd_topic`、`resp_topic`、`mqtt_en`、`uart_en`、`raw_auto`、`single_uart_split`、`radar_fw`、`radar_fw_version`、`radar_cfg`、`fw`、`raw_data_topic`、`raw_resp_topic`，以及 host 模式下的 `raw_cmd_topic`。
...
`radar status` 会返回保存下来的 `start_mode` 和 `supported_start_modes`；`device hi.fw.boot_mode` 则表示当前这次会话真实使用的雷达启动路径。
