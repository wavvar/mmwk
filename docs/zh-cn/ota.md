...
本指南仅用于已经运行当前公开 `mmwk_sensor_bridge` 固件包设备的 OTA 升级。

## 适用范围

- 仅适用于已运行设备的 OTA 流程。
- 使用 `../firmwares/esp/<board>/` 下的已发布 `mmwk_sensor_bridge` 产物。
- 不包含出厂刷机与包构建说明。
...
- 设备可通过 UART 访问，且已运行当前公开 `mmwk_sensor_bridge` 固件包。
...
请使用 `server.sh --device-ota --device-ota-board <board>` 发布 OTA 固件。
