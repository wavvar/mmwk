...
If `--broker` is absent and `MMWK_SERVER_MQTT_URI` is unset, `mmwk_raw.sh` auto-loads the broker from server.sh state. By default it checks `./build_output/local_server/server.env`, or you can point it somewhere else with `--server-state-dir`.
...
- `--raw-data-topic` / `--raw-resp-topic`: override the raw topics directly. If omitted, `mmwk_raw.sh` derives them from `--device-id`, and for restart/reboot triggers it can also fall back to runtime-reported topics.
